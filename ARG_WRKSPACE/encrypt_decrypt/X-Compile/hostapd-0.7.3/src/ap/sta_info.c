...
void ap_handle_timer(void *eloop_ctx, void *timeout_ctx)
{
	...
	printf("RAMESH: ITS IN ap handle timer\n");
	...
	if ((sta->flags & WLAN_STA_ASSOC) &&
		...
		int inactive_sec;
		printf("RAMESH: ITS IN ap handle timer 1\n");
		...
		if (inactive_sec == -1) {
			...
			printf("RAMESH: ITS IN ap handle timer 2\n");
	...
	if ((sta->flags & WLAN_STA_ASSOC) &&
		...
		printf("RAMESH: ITS IN ap handle timer 3\n");
	...
	if (sta->timeout_next == STA_NULLFUNC &&
		...
		printf("RAMESH: ITS IN ap handle timer 4\n");
		...
		printf("RAMESH: ITS IN ap handle timer 5\n");
	...
	case STA_NULLFUNC:
		...
		break;
	case STA_DISASSOC:
		printf("RAMESH: ITS IN ap handle timer 6\n");
		sta->flags &= ~WLAN_STA_ASSOC;
	...
	case STA_DEAUTH:
	case STA_REMOVE:
		printf("RAMESH: ITS IN ap handle timer 7\n");
		hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_IEEE80211,
			       HOSTAPD_LEVEL_INFO, "deauthenticated due to "
			       "inactivity");
		if (!sta->acct_terminate_cause)
...
void ap_sta_disassociate(struct hostapd_data *hapd, struct sta_info *sta,
			 u16 reason)
{
	printf("RAMESH: ITS IN ap_sta_disassociate\n");
...
void ap_sta_deauthenticate(struct hostapd_data *hapd, struct sta_info *sta,
			   u16 reason)
{
	printf("RAMESH: ITS IN ap sta_deauthenticate\n");
