					  ...
					  union wpa_event_data *data)
{
	...
	if (wpa_s->sme.auth_alg == WPA_AUTH_ALG_FT) {
		...
		if (wpa_drv_get_bssid(wpa_s, bssid) < 0 ||
	        ...
	        wpa_printf(MSG_DEBUG, "RAMESH: wpa_supplicant_deauthenticate 3(events.c)");
					 ...
					 union wpa_event_data *data)
{
	...
	if ((wpa_s->last_michael_mic_error &&
	    ...
	    wpa_printf(MSG_DEBUG, "RAMESH: wpa_supplicant_deauthenticate 4(events.c)");
			  ...
			  union wpa_event_data *data)
{
	...
	case EVENT_AUTH:
		{
		wpa_printf(MSG_DEBUG, "MA_FRAMEWORK-RAMESH: EVENT_AUTH\n");
	...
	case EVENT_SCAN_RESULTS:
		wpa_printf(MSG_DEBUG, "event_scan_results\n");
	...
	case EVENT_RX_MGMT:
		if (wpa_s->ap_iface == NULL)
			break;
		wpa_printf(MSG_DEBUG, "EVENT_RX_MGMT\n");
	...
	case EVENT_MLME_RX: {
		wpa_printf(MSG_DEBUG, "EVENT_MLME_RX\n");
	...
	case EVENT_EAPOL_RX:
		wpa_printf(MSG_DEBUG, "EVENT_EAPOL_RX\n");
