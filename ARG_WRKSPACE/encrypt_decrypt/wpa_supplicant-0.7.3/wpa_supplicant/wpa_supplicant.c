					   ...
					   struct wpa_ssid *ssid)
{
	...
	case WPA_CIPHER_TKIP:
		...
		wpa_printf(MSG_DEBUG,"*******cipher tkip******");
		break;
	default:
...
{
	if (wpa_s->drv_priv) {
	    wpa_printf(MSG_DEBUG, "RAMESH: wpa_supplicant_deauthenticate 2(wpa_supplicant.c)");
...
struct wpa_global * wpa_supplicant_init(struct wpa_params *params)
{
	...
	if (ret) {
		wpa_printf(MSG_ERROR, "SAI_ANU:Failed to register EAP methods");
		if (ret == -2)
