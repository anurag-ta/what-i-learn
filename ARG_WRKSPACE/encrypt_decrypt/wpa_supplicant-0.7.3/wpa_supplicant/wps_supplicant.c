...
int wpas_wps_eapol_cb(struct wpa_supplicant *wpa_s)
{
	if (!wpa_s->wps_success &&
	    ...
	    wpa_printf(MSG_DEBUG, "RAMESH: wpa_supplicant_deauthenticate 5(wps_supplicant.c)");
	...
	if (wpa_s->key_mgmt == WPA_KEY_MGMT_WPS && wpa_s->current_ssid &&
	    ...
	    wpa_printf(MSG_DEBUG, "RAMESH: wpa_supplicant_deauthenticate 6(wps_supplicant.c)");
		wpa_supplicant_deauthenticate(wpa_s,
					      WLAN_REASON_DEAUTH_LEAVING);
		wpa_s->after_wps = 5;
	...
	if (wpa_s->key_mgmt == WPA_KEY_MGMT_WPS && wpa_s->current_ssid) {
	    ...
	    wpa_printf(MSG_DEBUG, "RAMESH: wpa_supplicant_deauthenticate 7(wps_supplicant.c)");
		wpa_supplicant_deauthenticate(wpa_s,
					      WLAN_REASON_DEAUTH_LEAVING);
		return 1;
	}
